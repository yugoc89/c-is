...
if (version_compare(PHP_VERSION, '5.1.0', '>=')) {//PHP5.1.0以上の場合のみタイムゾーンを定義
	date_default_timezone_set('Europe/London');//タイムゾーンの設定（日本以外の場合には適宜設定ください）
}
/*-------------------------------------------------------------------------------------------------------------------
* ★以下設定時の注意点　
...
$re_subject = "Thank you for contacting us";
...
if(empty($errm)){
	foreach($_POST as $key=>$val) {
		...
		if($key == $Email && $mail_check == 1 && !empty($val)){
			if(!checkMail($val)){
				$errm .= "<p class=\"error_messe\">【".$key."】is not a valid E-mail address</p>\n";
				$empty_flag = 1;
			}
		}
	}
...
<?php if($empty_flag == 1){ ?>
<div align="center">
<h4>Some error has been detected on the form. Please click the back button on your browser and edit the content of the form.</h4>
<?php echo $errm; ?><br /><br /><input type="button" value=" Back to form " onClick="history.back()">
</div>
<?php }else{ ?>
<h3>Confirm</h3>
<p align="center">Please click 'Confirm' button if the following content is correct.</p>
...
<input type="button" value="Back to form" onClick="history.back()"></p>
</form>
<?php } ?>
</div><!-- /formWrap -->
<!-- ▲ *********** 送信内容確認部　※編集は自己責任で ************ ▲-->
...
<br /><br /><input type="button" value=" Back to the form " onClick="history.back()">
</div>
</body>
</html>
<?php }else{ ?>
Thank you for sending a message to us.<br />
We'll replay as soon as possible.<br /><br />
<a href="<?php echo $site_top ;?>">Go back home&raquo;</a>
</div>
...
//確認画面無しの場合の表示、指定のページに移動する設定の場合、エラーチェックで問題が無ければ指定ページヘリダイレクト
	...
	if($empty_flag == 1){ ?>
<div align="center"><h4>Some error has been detected on the form. Please click the back button on your browser and edit the content of the form.</h4><div style="color:red"><?php echo $errm; ?></div><br /><br /><input type="button" value=" 前画面に戻る " onClick="history.back()"></div>
<?php 
...
function connect2val($arr){
	...
	foreach($arr as $key => $val){
		if($key === 0 || $val == ''){//配列が未記入（0）、または内容が空のの場合には連結文字を付加しない（型まで調べる必要あり）
			$key = '';
		}elseif(strpos($key,"yen") !== false && $val != '' && preg_match("/^[0-9]+$/",$val)){
...
function mailToAdmin($arr,$subject,$mailFooterDsp,$mailSignature,$encode,$confirmDsp){
	$adminBody="You've got an email from 「".$subject."」\n\n";
	$adminBody .="＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n\n";
	$adminBody.= postToMail($arr);//POSTデータを関数からセット
	$adminBody.="\n＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n";
	$adminBody.="Date：".date( "Y/m/d (D) H:i:s", time() )."\n";
	$adminBody.="IP address：".@$_SERVER["REMOTE_ADDR"]."\n";
	$adminBody.="Host name：".getHostByAddr(getenv('REMOTE_ADDR'))."\n";
	if($confirmDsp != 1){
		$adminBody.="Page URL：".@$_SERVER['HTTP_REFERER']."\n";
	}else{
		$adminBody.="Page URL：".@$arr['httpReferer']."\n";
	}
...
function mailToUser($arr,$dsp_name,$remail_text,$mailFooterDsp,$mailSignature,$encode){
	...
	$userBody.="\n＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝\n\n";
	$userBody.="Date：".date( "Y/m/d (D) H:i:s", time() )."\n";
...
function requireCheck($require){
	...
	foreach($require as $requireVal){
		...
		foreach($_POST as $key => $val) {
			if($key == $requireVal) {
				...
				if(is_array($val)){
					...
					if($connectEmpty > 0){
						$res['errm'] .= "<p class=\"error_messe\">【".h($key)."】is necessary.</p>\n";
						$res['empty_flag'] = 1;
					}
				}
				//デフォルト必須チェック
		...
		if($existsFalg != 1){
				$res['errm'] .= "<p class=\"error_messe\">【".$requireVal."】is blank.</p>\n";
				$res['empty_flag'] = 1;
		}
	}
